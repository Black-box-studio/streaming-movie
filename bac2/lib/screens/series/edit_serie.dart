...
class EditSerie extends StatefulWidget {
  final idSerie;
  final isSerie;
  EditSerie({@required this.idSerie, @required this.isSerie});

  @override
  _EditSerieState createState() => _EditSerieState();
}

class _EditSerieState extends State<EditSerie> {
  var type;
  ...
  Future<void> getDataMovie() async {
    final _serie = await Database.getMovieData(idMovie: widget.idSerie);

    setState(() {
      _name.text = _serie['name'];
      _summary.text = _serie['summary'];
      _date.text = _serie['date'];
      _firstAired.text = _serie['firstAired'];
      _status.text = _serie['status'];
      _minutes.text = _serie['time'];
      _format.text = _serie['format'];
      _source.text = _serie['source'];
      _imageCover = _serie['image'];
      _season.text = _serie['season'];
    });

    getStudios(data: _serie['studio']);
    getCategories(data: _serie['categories']);
    getTrailerList(data: _serie['trailers']);
    getCharacterList(data: _serie['characters']);
  }

  @override
  void initState() {
    getDataMovie();
    type = widget.isSerie ? 'Serie' : 'Show';
    super.initState();
  }
                  ...
                  Center(
                    child: InkWell(
                      onTap: () async {
                        ...
                        final mUplaod = await Update.updateSeire(
                          idMovie: widget.idSerie,
